...
void GranularSampler::setup(Sample & _sample, int x, int y)
{
	...
	pointStart_frame = 0;
	pointEnd_frame = min(loopMaxLength_frames, sample.getLength());
	...
	startPointDraggable.position[0] = x;
	//drawingCenter[0] = x;
	drawingCenter[1] = y;
}
...
void GranularSampler::draw()
{
	...
	ofTranslate(drawingCenter[0], drawingCenter[1]);
		...
		if (startPointDraggable.isUnderMouse) {
			//draw original wave
			ofSetColor(0, 50, 50);
			ofBeginShape();
...
void GranularSampler::setStartFrame(long to) {
	...
	sample.snapPoints(pointStart_frame, pointStart_frame);
